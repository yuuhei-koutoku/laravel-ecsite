                        ...
                        <div class="p-2 w-3/4 mx-auto">
                            @foreach($inquiries as $inquiry)
                                @if ($inquiry->admin === 0)
                                    ...
                                    @else
                                        <div class="bg-gray-100 rounded-lg break-words px-2 py-1 mb-8 text-xs text-center mx-20 lg:mx-40">
                                            メッセージの送信を取り消しました
                                        </div>
                                    @endif
                                @else
                                    <div class="bg-gray-100 rounded-lg text-sm break-words px-2 py-1 mb-8 mr-20 lg:mr-40">
                                        {{ $inquiry->message }}
                                    </div>
                                @endif
                            @endforeach
