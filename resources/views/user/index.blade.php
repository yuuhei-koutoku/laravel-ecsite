    ...
    <x-slot name="header">
        ...
        <form method="get" action="{{ route('user.items.index') }}">
            <div class="lg:flex lg:justify-around">
                <div class="sm:flex items-center my-2 lg:my-0">
                    <select name="category" class="mb-2 sm:mb-0 sm:mr-2">
                    ...
                    <div class="flex space-x-2 items-center">
                        ...
                        <div>
                            <button class="text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded">検索</button>
                        </div>
                    </div>
                </div>
                ...
                <div class="p-2 text-gray-900">
                    <div class="flex flex-wrap">
                        @foreach ($products as $product)
                            <div class="w-full sm:w-1/2 md:w-1/3 lg:w-1/4 p-1 xl:p-2">
                                <a href="{{ route('user.items.show', ['item' => $product->id]) }}">
                                    <div class="border rounded-md p-1 xl:p-2">
                                        <x-thumbnail filename="{{$product->filename ?? ''}}" type="products" />
                                        <div class="mt-4">
                                            <h3 class="text-gray-500 text-xs tracking-widest title-font mb-1">{{ $product->category }}</h3>
                                            <h2 class="text-gray-900 title-font text-xs lg:text-sm xl:text-base font-medium">{{ $product->name }}</h2>
                                            <p class="text-xs lg:text-sm mt-1">{{ number_format($product->price) }}<span class="text-xs lg:text-sm text-gray-700">円(税込)</span></p>
                                        </div>
                                    </div>
                                </a>
                            </div>
